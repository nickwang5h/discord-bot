...
- AI/HN 日报读取 Top 30；
- 综合日报每个 RSS 最多 8 条，最终跨来源精选 6 条；
- 高级新闻每个 RSS 最多 4 条；
- 高级新闻在送入模型前使用一次缓存读取批量去重。

综合日报把发布方、标题、清洗后的 RSS 摘要和发布时间交给模型，但不交付链接。模型只返回
严格 JSON 中的候选 ID 与一句中文摘要；程序校验数量、ID、分类覆盖和摘要，再从原始候选
恢复标题、发布方及 HTTPS 链接。因此模型不能伪造来源或链接，也不能在证据不足时把外部
知识写成新闻事实。最终正文按完整条目组装并预留 Discord 长度余量，不再从 4000 字符处
硬截断半条新闻。当前只在单轮候选中按 URL 去重，早间与午后简报之间还没有已投递记忆，
所以跨轮重复仍是已知边界。
