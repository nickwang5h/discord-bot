...
所有请求默认限制 4096 个输出 token。综合新闻选择和高级新闻打分分别限制为 3000 输出 token；高级新闻每批最多 8 条，避免免费模型的 prompt 与 completion 因单次请求过大而触发限额。综合新闻另外受 Discord 单次消息 embed 总字符预算约束。
...
- AI/HN 日报读取 Top 30；
- 综合日报每个 RSS 最多 8 条，所有候选按发布方交错后再筛选；
- 高级新闻每个 RSS 最多 4 条；
- 高级新闻在送入模型前使用一次缓存读取批量去重。

综合日报把发布方、标题、清洗后的 RSS 摘要和发布时间交给模型，但不交付链接。模型只返回
严格 JSON 中的候选 ID、`core`/`breadth` 用途和一句中文摘要；程序校验 ID、类型、摘要及
渲染成本，再从原始候选恢复标题、发布方及 HTTPS 链接。因此模型不能伪造来源或链接，也
不能在证据不足时把外部知识写成新闻事实。选择不设固定条数或强制分类配额：同一事件只留
一条，除非另一来源提供了不同的实质事实或视角；真正增加当日态势理解或视野广度的候选才
进入输出。

正式投递后的 URL、标题、RSS 摘要、内容指纹、发布方和分类保留 48 小时，最多 200 条。
下一轮先排除 URL 与内容指纹都未变化的条目，再把最近标题和摘要作为无链接历史上下文交给
模型；同一 URL 的 RSS 证据发生变化时仍可作为实质进展重新评估。过期记录在下一次写入时
自动删除。输出使用关键变化与视野扩展两个 embed，但仍只调用一次
...
高级新闻 JSON 分析直接调用失败即抛异常的 `generate_ai()`，不会经过可能返回用户提示文本的兼容接口。代码会校验 `news` 为数组，只接受原始 batch 中存在的 URL，恢复原始标题/来源，将分数限制在 0-1 后再写入缓存。启动时会移除缺少当前评分字段的旧 schema 缓存。已投递记录保留在既有最多 150 条的缓存中并标记为 pushed，直到被更新条目自然淘汰；这样每小时抓取不会立即重新分析刚刚投递的 URL。
