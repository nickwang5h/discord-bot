    ...
    def update(cache: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for source_item in new_items:
            ...
            if url and url in existing_urls:
                previous = next(entry for entry in cache if entry.get("url") == url)
                # Backfill raw evidence on re-fetch, preserving delivery identity and old fields.
                if not previous.get("pushed") and "content" not in previous and "content" in item:
